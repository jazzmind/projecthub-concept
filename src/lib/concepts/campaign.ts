...
export class CampaignConcept {
  ...
  async create(input: {
    ...
    try {
      // Check if campaign identifier already exists
      const existing = await this.prisma.campaign.findFirst({
        where: { campaign: input.campaign }
      });

      if (existing) {
        return { error: "Campaign with this identifier already exists" };
      }
  ...
  async delete(input: { campaign: string }): Promise<{ success: boolean } | { error: string }> {
    try {
      const existing = await this.prisma.campaign.findFirst({
        where: { campaign: input.campaign }
      });
  ...
  // Queries
  async _getById(input: { campaign: string }): Promise<Campaign[]> {
    try {
