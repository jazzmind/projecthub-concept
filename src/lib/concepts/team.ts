...
export class TeamConcept {
  ...
  async create(input: {
    ...
    try {
      ...
      // Check if team with this identifier already exists
  ...
  async delete(input: { team: string }): Promise<{ success: boolean } | { error: string }> {
    try {
  ...
  // Queries
  async _getByTeam(input: { team: string }): Promise<Team[]> {
    try {
