...
export class RoleConcept {
  ...
  async activate(input: { name: string }): Promise<{ role: Role } | { error: string }> {
    try {
      const role = await this.prisma.role.findFirst({
        where: { name: input.name }
      });
  ...
  async deactivate(input: { name: string }): Promise<{ role: Role } | { error: string }> {
    try {
      const role = await this.prisma.role.findFirst({
        where: { name: input.name }
      });
  ...
  async delete(input: { name: string }): Promise<{ success: boolean } | { error: string }> {
    try {
      const role = await this.prisma.role.findFirst({
        where: { name: input.name }
      });
  ...
  // Queries
  async _getByName(input: { name: string }): Promise<Role[]> {
    try {
      const role = await this.prisma.role.findFirst({
        where: { name: input.name }
      });
  ...
  async _hasPermission(input: { name: string; resource: string; action: string }): Promise<boolean[]> {
    try {
      const role = await this.prisma.role.findFirst({
        where: { name: input.name }
      });
